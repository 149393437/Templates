...
class ___FILEBASENAME___: FIFinderSync {
    ...
    override init() {
        ...
        FIFinderSyncController.defaultController().directoryURLs = NSSet(object: self.myFolderURL)
    ...
    @IBAction func sampleAction(sender: AnyObject?) {
        ...
        NSLog("sampleAction: menu item: %@, target = %@, items = ", (item as NSMenuItem).title, target.filePathURL!)
        for obj: AnyObject in items {
            NSLog("    %@", (obj as NSURL).filePathURL!)
        }
    }
